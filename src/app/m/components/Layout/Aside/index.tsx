...
import {
  CloseButton,
  Layout,
} from '@/app/m/components/Layout/Aside/Aside.styled';
import AsideMenu from '@/app/m/components/Layout/Aside/Menu';
...
function Menu({ isOpen, setIsOpen }: Props) {
  ...
  const categoryMenu = categories.map(category => ({
    text: category.name,
    link: `/m/categories/${category.id}`,
  }));

  const menu = [
    {
      text: '운세 다시보기',
      link: '/m/contents/replays',
    },
    {
      text: '운세 전체보기',
      link: '/m/contents',
