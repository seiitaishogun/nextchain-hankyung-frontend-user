...
import { ShareButton } from '@/app/m/contents/[id]/result/components/Footer/styled';
import Share from '@/app/m/components/Share';
...
function ContentShare({ isShare, content, purchase }: Props) {
  ...
  const { handleShare, renderShare } = useShare({
    kakaoOptions: {
      name: content.name,
      image: content.banner || undefined,
      description,
    },
  ...
  if (isShare) {
    return (
      <ShareButton
        type="button"
        className="btn-share"
        onClick={() => router.push(`/m/contents/${content.id}`)}
      >
        {content.name} 보러가기
      </ShareButton>
    );
