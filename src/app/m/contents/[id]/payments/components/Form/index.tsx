...
import FormStep from '@/app/m/contents/[id]/payments/components/Step';
import { PaymentFormProps } from '@/app/m/contents/[id]/payments/components/Form/types';
import {
  mid,
  returnURL,
  ediDate,
} from '@/app/m/contents/[id]/payments/components/Form/constants';
...
function PaymentForm({ content }: PaymentFormProps) {
    ...
    paymentHashMutate.mutate(
      ...
      },
      {
        onSuccess: res => {
          ...
          if (currentPrice === 0) {
            router.replace(`/m/contents/${content.id}/result/${purchaseId}`);
            return;
          }
  ...
  return (
    <FormProvider {...methods}>
      <div>
        <FormStep content={content} handleFormSubmit={handleFormSubmit} />
